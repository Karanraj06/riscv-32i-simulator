...
def memory_access() -> None:
    ...
    if de.MemOp == 0:
        ...
        if de.func3 == "010":  # for SW
            ...
            f.write(
                f"MEMORY: Store {de.bin_to_dec(dec_to_bin(de.op2)[24:].zfill(32))} at address {ex.aluResult}\n"
            )

        elif de.func3 == "001":  # for SH
            data_memory[ex.aluResult] = dec_to_bin(de.op2)[16:].zfill(32)
            f.write(
                f"MEMORY: Store {de.bin_to_dec(dec_to_bin(de.op2)[16:].zfill(32))} at address {ex.aluResult}\n"
            )
